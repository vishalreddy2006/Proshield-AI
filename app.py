...
if "label" in subset_cols:
    styled_logs = styled_logs.map(_colour_label, subset=["label"])
if "severity" in subset_cols:
    styled_logs = styled_logs.map(_colour_severity, subset=["severity"])
...
if report["suspicious_events"]:
    ...
    if "severity" in anom_cols:
        styled_anom = styled_anom.map(_colour_severity, subset=["severity"])
...
if incidents:
    ...
    if "severity" in incident_cols:
        styled_incidents = styled_incidents.map(_colour_severity, subset=["severity"])
